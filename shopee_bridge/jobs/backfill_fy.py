...
def run(company: str, fiscal_year_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        ShopeeSyncLog.write_log("backfill_fy", f"fy:{fiscal_year_name}", status, meta=summary)
    except Exception as exc:  # pragma: no cover
        summary["error"] = str(exc)
        ShopeeSyncLog.write_log("backfill_fy", f"fy:{fiscal_year_name}", "fail", message=str(exc))
    return summary
